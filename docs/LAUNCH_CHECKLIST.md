...
| T15 | Driver notification preferences DB migration `00044_driver_notification_preferences.sql` applied before launch; Driver settings now save through `/api/driver/notification-preferences`. GET falls back to safe defaults when the table is not available, but persisted PATCH saves require the migration. | Eng | |
