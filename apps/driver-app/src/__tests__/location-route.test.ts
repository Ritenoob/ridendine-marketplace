...
jest.mock('@ridendine/db', () => ({
  ...
  getDeliveryContextForDriverLocation: (client: { from: (table: string) => any }, deliveryId: string) =>
    table(client, 'deliveries')
      .select('id, driver_id, status, order_id')
      .eq('id', deliveryId)
      .single(),
  getDeliveryEtaSnapshotForDriverLocation: (client: { from: (table: string) => any }, deliveryId: string) =>
    table(client, 'deliveries')
      .select('eta_pickup_at, route_to_dropoff_polyline')
      .eq('id', deliveryId)
      .maybeSingle(),
  getDeliveryOwnerForDriverLocation: (client: { from: (table: string) => any }, deliveryId: string) =>
    table(client, 'deliveries')
      .select('id, driver_id')
      .eq('id', deliveryId)
      .maybeSingle(),
  getOrderPublicStageForDriverLocation: (client: { from: (table: string) => any }, orderId: string) =>
    table(client, 'orders')
      .select('public_stage')
      .eq('id', orderId)
      .maybeSingle(),
  insertDeliveryTrackingLocation: (
    ...
    table(client, 'delivery_tracking_events').insert({
  ...
  insertDriverLocation: (
    ...
    table(client, 'driver_locations').insert({
      driver_id: payload.driverId,
      lat: payload.lat,
      lng: payload.lng,
      accuracy: payload.accuracy ?? null,
  ...
  upsertDriverCurrentLocation: (
    ...
    table(client, 'driver_presence').upsert(
      {
        driver_id: payload.driverId,
