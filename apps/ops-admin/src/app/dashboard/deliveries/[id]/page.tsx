      ...
      <div className="mx-auto max-w-6xl space-y-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <Badge className="bg-info/20 text-info">{detail.status}</Badge>
            <Badge
              className={
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <Card className="border-border bg-surface p-6">
            ...
            <div className="mt-4 space-y-3 text-sm">
              ...
              <div className="flex justify-between gap-4">
                <span className="text-textMuted">Storefront</span>
            ...
            <div className="mt-4 flex flex-wrap gap-4 text-sm">
              <Link href={`/dashboard/orders/${detail.order.id}`} className="text-primary hover:underline">
                View order detail &rarr;
              </Link>
              {detail.driver && (
                <Link href={`/dashboard/drivers/${detail.driver.id}`} className="text-primary hover:underline">
          ...
          <Card className="border-border bg-surface p-6">
            ...
            <div className="mt-4 grid gap-4">
              <div className="rounded-lg bg-surface p-4">
                <p className="text-xs uppercase tracking-wide text-textMuted">Pickup</p>
                <p className="mt-2 text-sm text-white">{detail.pickup.address}</p>
              </div>
              <div className="rounded-lg bg-surface p-4">
                <p className="text-xs uppercase tracking-wide text-textMuted">Dropoff</p>
                <p className="mt-2 text-sm text-white">{detail.dropoff.address}</p>
              </div>
              <div className="grid gap-4 sm:grid-cols-3">
                <div className="rounded-lg bg-surface p-4">
                  <p className="text-xs uppercase tracking-wide text-textMuted">Delivery fee</p>
                  <p className="mt-2 text-lg font-semibold text-white">
                    {formatMoney(detail.payout.deliveryFee)}
                  </p>
                </div>
                <div className="rounded-lg bg-surface p-4">
                  <p className="text-xs uppercase tracking-wide text-textMuted">Driver payout</p>
        ...
        <div className="grid gap-6 xl:grid-cols-[2fr,1fr]">
          <Card className="border-border bg-surface p-6">
            ...
            <div className="mt-4 space-y-3">
              {detail.assignmentAttempts.length === 0 ? (
                <p className="text-sm text-textMuted">No assignment attempts recorded yet.</p>
              ) : (
                detail.assignmentAttempts.map((attempt) => (
                  <div key={attempt.id} className="rounded-lg bg-surface p-4">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      ...
                      <Badge className="bg-surfaceMuted text-textSubtle">{attempt.response}</Badge>
                    </div>
                    {attempt.declineReason && (
                      <p className="mt-2 text-sm text-textMuted">{attempt.declineReason}</p>
                    )}
                  </div>
                ))
              )}
          ...
          <Card className="border-border bg-surface p-6">
            ...
            <div className="mt-4 space-y-3 text-sm">
              <div className="rounded-lg bg-surface p-4">
                <p className="text-xs uppercase tracking-wide text-textMuted">Assigned driver</p>
                <p className="mt-2 text-white">{detail.driver?.name ?? 'No driver assigned'}</p>
                {detail.driver?.phone && (
                  <p className="mt-1 text-textMuted">{detail.driver.phone}</p>
                )}
              ...
              <div className="rounded-lg bg-surface p-4">
                <p className="text-xs uppercase tracking-wide text-textMuted">Escalation</p>
        ...
        <div className="grid gap-6 xl:grid-cols-[2fr,1fr]">
            ...
            <div className="mt-4 space-y-3">
              {detail.eventTimeline.length === 0 ? (
                ...
                detail.eventTimeline.map((event) => (
                  <div key={event.id} className="rounded-lg bg-surface p-4">
                    <div className="flex items-center justify-between gap-4">
                      <p className="text-white">{event.type}</p>
                      <span className="text-xs text-textMuted">{formatDate(event.timestamp)}</span>
                    </div>
                    {event.note && <p className="mt-2 text-sm text-textMuted">{event.note}</p>}
                  </div>
                ))
              )}
            </div>
          ...
          <Card className="border-border bg-surface p-6">
            ...
            <div className="mt-4 space-y-4">
              <div className="rounded-lg bg-surface p-4">
                <p className="text-xs uppercase tracking-wide text-textMuted">Ops notes</p>
                <div className="mt-3 space-y-3">
                  {detail.opsNotes.length === 0 ? (
                    <p className="text-sm text-textMuted">No ops notes recorded.</p>
                  ) : (
                    detail.opsNotes.map((note) => (
                      <div key={note.id}>
                        <p className="text-sm text-white">{note.content}</p>
                        <p className="text-xs text-textMuted">{formatDate(note.createdAt)}</p>
                      </div>
                    ))
                  )}
                </div>
              ...
              <div className="rounded-lg bg-surface p-4">
                <p className="text-xs uppercase tracking-wide text-textMuted">Tracking breadcrumbs</p>
                <div className="mt-3 space-y-2">
                  {detail.trackingBreadcrumbs.length === 0 ? (
                    <p className="text-sm text-textMuted">No tracking breadcrumbs recorded.</p>
                  ) : (
                    detail.trackingBreadcrumbs.slice(0, 5).map((point) => (
                      <div key={point.id} className="text-sm text-textSubtle">
