...
export default async function FinanceDriverAccountsPage() {
  ...
  let error = false;
  try {
    rows = (await listPlatformAccountsByType(admin, 'driver_payable')) as unknown as {
      owner_id: string;
  ...
  const drivers = (
    driverIds.length > 0 ? await listDriverNameRefs(admin, driverIds).catch(() => []) : []
    ...
    <DashboardLayout>
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div>
          ...
          <p className="mt-1 text-textMuted">Balances derived from ledger_entries (driver_payable).</p>
        </div>
        {error ? (
