...
export function DriversColumn({ drivers }: { drivers: OpsLiveDriverView[] }) {
  return (
    <div className="flex flex-col gap-2 rounded-lg border border-border bg-[#121c2c] p-3">
      <h3 className="text-sm font-semibold uppercase tracking-wide text-textMuted">Drivers</h3>
      <div className="flex max-h-[70vh] flex-col gap-2 overflow-y-auto">
        {drivers.map((d) => {
          ...
          return (
            <Card
              key={d.id}
              className={`border-border bg-surface p-3 ${
                stale ? 'border-warning/60' : ''
              } ${overloaded ? 'border-danger/50' : ''}`}
            >
                  ...
                  className={
                    d.presenceStatus === 'online'
                      ...
                      : d.presenceStatus === 'busy'
                        ...
                        : 'bg-surfaceMuted/40 text-textSubtle text-[10px]'
                  }
                >
                  {d.presenceStatus}
                </Badge>
              ...
              {overloaded && (
                <p className="mt-1 text-[11px] text-danger">High concurrent load</p>
              )}
