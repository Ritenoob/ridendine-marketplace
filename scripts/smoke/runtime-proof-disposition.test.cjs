...
test('dispositions every remaining page proof gap', () => {
  ...
  assert.equal(summary.pageTotals.total, 94);
  assert.equal(summary.pageTotals.proofCovered, 93);
...
test('dispositions every remaining API proof gap', () => {
  ...
  assert.equal(summary.apiTotals.total, 133);
  assert.equal(summary.apiTotals.proofCovered, 133);
...
test('generates markdown proof disposition docs with zero unresolved gaps', () => {
  ...
  assert.ok(markdown.includes('| Pages | 94 | 93 | 1 | 1 | 0 |'));
  assert.ok(markdown.includes('| API route handlers | 133 | 133 | 0 | 0 | 0 |'));
